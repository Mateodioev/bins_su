 ...
 * CurlX Lib - v0.0.4
 * @author D3VBL4CK
 * @link https://github.com/devblack/curlx
 */
class CurlX
{
    const VERSION = '0.0.4';
    ...
    private static function SetCookie(string $file) : void 
    {
        ...
        // PHP7.4+
        self::$cookie_file = sprintf("%s/Cache/kirari_%s.txt", self::$current_dir, $file);
    ...
    {   
        $parse_string = $decode ? explode($end, explode($start, base64_decode($str))[1])[0] : explode($end, explode($start, $str)[1])[0];
        return trim($parse_string);
    }
